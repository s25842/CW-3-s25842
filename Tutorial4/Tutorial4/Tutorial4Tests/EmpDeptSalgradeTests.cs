...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        List<Emp> result = (from emp in emps
            where emp.Job == "SALESMAN" select emp).ToList();
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = (from emp in emps
            where emp.DeptNo == 30 orderby emp.Sal descending select emp).ToList(); 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = (from emp in emps group emp by emp.DeptNo into grouped select new {DeptNo=grouped.Key,Count=grouped.Count()}).ToList(); 
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        var emps = Database.GetEmps();
         ...
         var result = from emp in emps group emp by emp.DeptNo into grouped select new {DeptNo = grouped.Key,AvgSal = grouped.Average(g => g.Sal)}; 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        var emps = Database.GetEmps();
         ...
         var result = (from emp in emps where (emp.Sal >(from emp2 in emps where emp2.DeptNo == emp.DeptNo select emp2.Sal).Average()) select emp.EName); 
