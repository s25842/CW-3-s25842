...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnMinSalaryInDept30()
    {
        ...
        decimal? minSalary = emps.Min(emp => emp.Sal);

        Assert.Equal(800, minSalary);//zmienilem z 1250 na 800 bo 800 to minSal
    }
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        var emps = Database.GetEmps();
         ...
         var result = from emp in emps join emp2  in emps on emp.Mgr equals emp2.EmpNo 
                 select new { Employee = emp.EName,Manager = emp2.EName };
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        var emps = Database.GetEmps();
         ...
         var result = from emp in emps select new {EName = emp.EName, Total = emp.Sal + (emp.Comm ?? 0)}; 
